...
describe("MovieTable tests", () => {
  ...
  const expectedHeaders = ["id", "Name", "Starring", "Director"];
  const expectedFields = ["id", "name", "starring", "director"];
  ...
  test("Has the expected column headers, content and buttons", () => {
    ...
    expect(screen.getByTestId(`${testId}-cell-row-1-col-name`)).toHaveTextContent("Inception");
  ...
  test("Edit button navigates to the edit page", async () => {
    ...
    const expectedMessage = `editCallback: {"id":2,"name":"San Andreas","starring":"Dwayne Johnson","director":"Brad Peyton"})`;
  ...
  test("Details button navigates to the details page", async () => {
    ...
    expect(console.log).toHaveBeenCalled();
    const message = console.log.mock.calls[0][0];
    const expectedMessage = `detailsCallback: {"id":2,"name":"San Andreas","starring":"Dwayne Johnson","director":"Brad Peyton"})`;
    expect(message).toMatch(expectedMessage);
    restoreConsole();
  });
  ...
  test("Delete button calls delete callback", async () => {
    ...
    fireEvent.click(deleteButton);
     ...
     const message = console.log.mock.calls[0][0];
     const expectedMessage = `deleteCallback: {"id":2,"name":"San Andreas","starring":"Dwayne Johnson","director":"Brad Peyton"})`;
     expect(message).toMatch(expectedMessage);
     restoreConsole();
  });
});
